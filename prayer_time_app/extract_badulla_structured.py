...
try:
    ...
    # Badulla / Monaragala is Zone 10
    target_pages = []
    for page in reader.pages:
        text = str(page.extract_text() or '')
        if 'BADULLA' in text.upper() or 'MONARAGALA' in text.upper() or 'ZONE: 10' in text.upper() or 'ZONE:10' in text.upper():
    ...
    # Format output as JS
    with open('badulla_prayer_data.js', 'w') as f:
        f.write('const BADULLA_PRAYER_DATA = ')
        json.dump(zone_data, f, indent=2)
        f.write(';\n')

    print("Parsed data saved to badulla_prayer_data.js")
