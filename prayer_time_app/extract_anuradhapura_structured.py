...
try:
    ...
    # Anuradhapura / Polonnaruwa is Zone 05
    target_pages = []
    for page in reader.pages:
        text = str(page.extract_text() or '')
        if 'ANURADHAPURA' in text.upper() or 'POLONNARUWA' in text.upper() or 'ZONE: 05' in text.upper() or 'ZONE:05' in text.upper():
    ...
    # Format output as JS
    with open('anuradhapura_prayer_data.js', 'w') as f:
        f.write('const ANURADHAPURA_PRAYER_DATA = ')
        json.dump(zone_data, f, indent=2)
        f.write(';\n')

    print("Parsed data saved to anuradhapura_prayer_data.js")
