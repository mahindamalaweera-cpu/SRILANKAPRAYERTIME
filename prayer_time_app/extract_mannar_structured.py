...
try:
    ...
    # Mannar / Puttalam is Zone 04
    target_pages = []
    for page in reader.pages:
        text = str(page.extract_text() or '')
        if 'MANNAR' in text.upper() or 'PUTTALAM' in text.upper() or 'ZONE: 04' in text.upper() or 'ZONE:04' in text.upper():
    ...
    # Format output as JS
    with open('mannar_prayer_data.js', 'w') as f:
        f.write('const MANNAR_PRAYER_DATA = ')
        json.dump(zone_data, f, indent=2)
        f.write(';\n')

    print("Parsed data saved to mannar_prayer_data.js")
