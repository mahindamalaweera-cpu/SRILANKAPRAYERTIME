...
try:
    ...
    # Batticaloa / Ampara is Zone 08
    target_pages = []
    for page in reader.pages:
        text = str(page.extract_text() or '')
        if 'BATTICALOA' in text.upper() or 'AMPARA' in text.upper() or 'ZONE: 08' in text.upper() or 'ZONE:08' in text.upper():
    ...
    # Format output as JS
    with open('batticaloa_prayer_data.js', 'w') as f:
        f.write('const BATTICALOA_PRAYER_DATA = ')
        json.dump(zone_data, f, indent=2)
        f.write(';\n')

    print("Parsed data saved to batticaloa_prayer_data.js")
